...
export const reducer = (
	...
	switch ( action.type ) {
		...
		case nodes.ActionTypes.UPDATE_NODE:
			...
			updatedNode.inputValues[action.payload.inputIndex] = action.payload.inputValue;
			return updateNodeAndDependents( state, updatedNode );
...
const updateNodeAndDependents = ( nodes: Map<number, NodeState>, currentNode: NodeState ): Map<number, NodeState> => {
	...
	return getConnectedNodes( nodes, currentNode )
		.reduce(
...
const getConnectedNodes = ( nodes: Map<number, NodeState>, currentNode: NodeState ) => {
