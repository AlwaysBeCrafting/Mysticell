...
describe("graph resolver", () => {
  it("resolves a graph with only primitives", async () => {
    ...
    expect(result).toHaveLength(
      testDoc.formulas["FORMULA-addTwice"].outputNames.length,
    );
    expect(result).toEqual([PARAMS.number(7), PARAMS.number(9)]);
  });
  it("resolves a graph that includes another one", async () => {
    const result = await resolveProperty(testDoc, "FORMULA-include");
    expect(result).toHaveLength(
      testDoc.formulas["FORMULA-include"].outputNames.length,
    );
    expect(result).toEqual([PARAMS.string("2"), PARAMS.number(7)]);
  });
  it("returns an error when the graph's contents loop", async () => {
    const result = await resolveProperty(testDoc, "FORMULA-cyclic");
    expect(result).toHaveLength(
      testDoc.formulas["FORMULA-cyclic"].outputNames.length,
    );
    expect(result[0]).toHaveProperty("type", "error");
  });
});
