...
test("add function returns sum of two numbers", () => {
	expect(add(P.number(3), P.number(2))[0]).toHaveProperty("value", 5);
});

test("add function converts a numeric string to a number", () => {
	expect(add(P.number(3), P.string("2"))[0]).toHaveProperty("value", 5);
});
...
test("add function treats empty as identity", () => {
	expect(add(P.number(3), P.empty())[0]).toHaveProperty("value", 3);
});
...
test("add function returns an error if given an array", () => {
	expect(add(P.number(3), P.array([]))[0]).toHaveProperty("type", "error");
});
