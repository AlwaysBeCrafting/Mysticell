...
const isPrimitive = (prototype: NodePrototype): prototype is PrimitiveNodePrototype => (
	prototype.id.startsWith("primitive")
);
const isGraph = (prototype: NodePrototype): prototype is GraphNodePrototype => (
	isFunction(prototype) || isProperty(prototype)
);
const isFunction = (prototype: NodePrototype): prototype is FunctionNodePrototype => (
	prototype.id.startsWith("function")
);
const isProperty = (prototype: NodePrototype): prototype is PropertyNodePrototype => (
	prototype.id.startsWith("property")
);
